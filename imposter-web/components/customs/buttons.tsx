...
export const AppButton = forwardRef<HTMLButtonElement, AppButtonProps>(({className, variant = "primary", children, asChild = false, ...props}, ref) => {
    ...
    return (
        <Comp ref={ref} {...props} className={`flex justify-center items-center rounded p-1 font-semibold cursor-pointer tracking-wider ${variants[variant]} ${className}`}>{children}</Comp>
    )
})
