...
def get_login_user(token:Annotated[str, Depends(oauth2_scheme)], session:Annotated[Session, Depends(get_session)]) -> LoginUser:
    ...
    try:
        if role == "player":
            ...
            return LoginUser(username=account.email, password=account.password, role=role)
        if role == "admin":
            admin = safecall(session.exec(select(AdminAccount).where(AdminAccount.admin_email == email)).first(), "AdminAccount", "email", email)
            return LoginUser(username=admin.admin_email, password=admin.admin_password, role=role)
